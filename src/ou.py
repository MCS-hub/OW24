...
class OU_distrib_modeler:
    ...
    def __init__(self, A, b, beta):
        ...
        assert (
            np.linalg.matrix_rank(self.A, tol=1e-6) == self.A.shape[0]
    ...
    def _get_var_param(self, t):
        return (
            2.0
            * (1.0 / self.beta)
            * self._U_rotate((1.0 - np.exp(-2.0 * self.T * t)) / (2.0 * self.T))
        )
...
class OU_tNormal(TD.MultivariateNormal):
    ...
    def _get_params(init_distrib, ou_distrib_modeler, t):
        ...
        F, g = torch.tensor(F, dtype=dtype).to(device), torch.tensor(g, dtype=dtype).to(
            device
        )
        Sigma = torch.tensor(ou_distrib_modeler._get_var_param(t), dtype=dtype).to(
            device
        )
...
def create_em_proxrec_samples(
    x0, pdf0, final_distrib, t_fin, t_stp, beta=1.0, verbose=False, **proxrec_params
):
    """
    ...
    x_fin, pdf_fin = torchBatchItoEulerProxrec(
        ...
        beta=beta,
        verbose=verbose,
        grad_pot_func=targ_grad_potential,
        **proxrec_params
    )
...
def create_em_samples(
    x0, final_distrib, t_fin, t_stp, n_samples, beta=1.0, return_init_spls=False
):
    """
...
def create_em_samples(
    ...
    return create_em_samples(
        ...
        n_samples,
        beta=beta,
        return_init_spls=return_init_spls,
    )


def generate_ou_target(dim, mean_scale=1.0, dtype=torch.float32, device="cpu"):
...
def get_ou_potential_func(
    ...
    if _type == "numpy":
        ...
        def ou_potential_func_dim_first(x):
            ...
            w = (1.0 / (2.0 * beta)) * np.sum(x_norm * np.dot(var_inv, x_norm), axis=0)
            return w
        ...
        def ou_potential_func_dim_first(x):
            ...
            w = (1.0 / (2.0 * beta)) * torch.sum(
                x_norm * torch.matmul(var_inv, x_norm), dim=0
            )
            return w
...
if __name__ == "__main__":
    ...
    mean, var = ou_d_m.get_distrib_params(X_0, 70.0)
