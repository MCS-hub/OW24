    ...
    def push_nograd(self, input):
        ...
        for i in range(0, len(input), self.batch_size):
            ...
            output.data[i : i + self.batch_size] = self.push(
                input[i : i + self.batch_size], create_graph=False, retain_graph=False
            ).data
        return output
        ...
        hessian = torch.cat(
            [
                torch.autograd.grad(
                    ...
                    retain_graph=True,
                    only_inputs=True,
                    grad_outputs=torch.ones(input.size()[0]).float().to(input),
                )[0][:, None, :]
...
class LinDenseICNN(GradNN):
    ...
    def __init__(
        ...
        self.W_layers = nn.ModuleList(
            [nn.Linear(in_dim, out_dim) for in_dim, out_dim in w_sizes]
        )

        self.A_layers = nn.ModuleList(
            [nn.Linear(self.in_dim, out_dim) for out_dim in _hidden]
        )
    ...
    def forward(self, input):
        ...
        return self.final_layer(z) + 0.5 * self.strong_convexity * (input**2).sum(
            dim=1
        ).reshape(-1, 1)
    ...
    def relaxed_convexity_regularization(self):
        ...
        for layer in self.W_layers:
            assert isinstance(layer, nn.Linear)
            regularizer += layer.weight.clamp(max=0.0).pow(2).sum()
        regularizer += self.final_layer.weight.clamp(max=0.0).pow(2).sum()
        return regularizer
...
class DenseICNN(GradNN):
        ...
        self.convex_layers = nn.ModuleList(
            [
                WeightTransformedLinear(
                    ...
                    out_features,
                    bias=False,
                    w_transform=self.conv_layers_w_trf,
                )
    ...
    def forward(self, input):
        ...
        for quadratic_layer, convex_layer in zip(
            self.quadratic_layers[1:], self.convex_layers
        ):
