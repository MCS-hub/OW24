...
def KL_targ_distrib(X, diff, targ, **kwargs):
    return (
        targ.log_prob(X).mean() - diff.log_prob(X, **kwargs).mean()
    ).detach()  # the first term is target's entropy


@dispatch(int, Diffusion, object)
def KL_targ_distrib(n, diff, targ, ret_targ_sample=False, **kwargs):
    X = targ.sample((n,))  # get samples X directly from target distribution
    if len(X.shape) == 1:
        X = X.view(-1, 1)
    if ret_targ_sample:  # return target samples
...
def KL_targ_distrib(
    n, em_kde_distrib, targ, try_true_entropy=False, ret_targ_sample=False
):
    kl = 0.0
    X = targ.sample((n,))
    if try_true_entropy:
...
def KL_train_distrib(
    em_sample, em_kde_distrib, targ, device="cpu", dtype=torch.float32
):
...
def train_diffusion_model(
    ...
    assert target_type in [
        "distribution",
        "data_posterior",
    ], f"target_type='{target_type}' is not known"
    kl_train = []
    ...
    for i in range(n_steps):
        ...
        if init_model is None:
            ...
            D.load_state_dict(
                ...
                if i == 0
                else diffusion_model.Ds[-1].state_dict()
            )  # initialse with the lattest model
        loss_history = []
        opt = torch.optim.Adam(D.parameters(), lr=lr, weight_decay=1e-9)

        def compute_loss_distribution(X, factor):  # factor=1/N
            X.requires_grad_(True)
            push_X = D.push(X)  # \nabla psi(X)
            neg_loss = (
                factor * diffusion_model.negative_entropy_gain(push_X, X).sum()
            )  # entropy loss
            w_loss = (
                factor * 0.5 * ((push_X - X) ** 2).sum(dim=1).sum()
            )  # Wasserstein distance
            if dc_discretization:
                targ_loss = -factor * G_component(push_X).sum()
            else:
                targ_loss = factor * target.log_prob(push_X).sum()  # potential loss
            loss = w_loss + step_size * (neg_loss - targ_loss)  # stepsize = h, or gamma
            return loss
        ...
        def compute_loss_data_posterior(X, factor):
            ...
            data_targ_loss = (
                target.len_dataset
                * factor
                * target.est_log_data_prob(push_X, S, reduction="sum")
            )
        ...
        for it in tqdm(range(step_iterations), disable=not verbose):
            ...
            if init_sampler is None:
                X = diffusion_model.sample(batch_size, nu=True)

            else:
                X_0 = init_sampler.sample(batch_size)
                X = diffusion_model.propagate(X_0, nu=True)

            loss_history.append(0.0)
            for i_prop in range(0, X.size(0), n_max_prop):
                X_prop = X[i_prop : i_prop + n_max_prop]
                c_loss = compute_loss(X_prop, 1.0 / X.size(0))
                c_loss.backward()
            ...
            D.convexify()
            opt.zero_grad()
        diffusion_model.Ds.append(D)  # Ds means many D's
        if X_test is not None:
            kl_train.append(
                KL_train_distrib(
                    X_test, diffusion_model, target
                ).item()  # KL between model dis and target dis on test data
            )
        if ret_loss_history:
...
def train_diffusion_model_dc_discretization(
    diffusion_model,
    init_model,
    model_params,
    target,
    ...
    for i in range(n_steps):
        ...
        def compute_loss_distribution(X, factor):  # factor=1/N
            ...
            targ_loss = factor * target.log_prob(push_X).sum()  # potential loss
            loss = w_loss + step_size * (neg_loss - targ_loss)  # stepsize = h
            return loss
        ...
        for it in tqdm(range(step_iterations), disable=not verbose):
            ...
            if init_sampler is None:
                X = diffusion_model.sample(
                    batch_size
                )  # sample Z from init distribution and propagrate to get X from current distribution \nu_{n+1}
            else:
                X_0 = init_sampler.sample(batch_size)
        ...
        if X_test is not None:
            kl_train.append(
                KL_train_distrib(X_test, diffusion_model, target).item()
            )  # KL between model dis and target dis on test data
        if ret_loss_history:
            cum_loss_history.append(loss_history)
...
def discretize_distrib(
    xs, distrib, dtype=torch.float32, np_dtype=np.float64, device="cpu"
):
...
def energy_based_distance(X, Y, dim=None):
    ...
    if isinstance(Y, torch.Tensor):
        Y = Y.cpu().numpy()
    A = np.mean(np_cdist(X, Y))
    B = np.mean(np_cdist(X, X))
    C = np.mean(np_cdist(Y, Y))
    return 2.0 * A - B - C
